...
#Rendija
hueco = 3
amplitud0[int((b-a)/3)*2][:int((b-a)/2)-hueco]= 0
amplitud0[int((b-a)/3)*2][int((b-a)/2)+hueco:]= 0
...
amplitud1[:,-1]=0
#Rendija
amplitud1[int((b-a)/3)*2][:int((b-a)/2)-hueco]= 0
amplitud1[int((b-a)/3)*2][int((b-a)/2)+hueco:]= 0
...
while t<int(iteraciones):
	...
	#Rendija
	amplitudFinal[int((b-a)/3)*2][:int((b-a)/2)-hueco]= 0
	amplitudFinal[int((b-a)/3)*2][int((b-a)/2)+hueco:]= 0
...
linea = ax.plot_surface(X,Y, AinTime[20],linewidth =0, color='c', cmap = cm.coolwarm)
